 ...
 * @link      http://pear.php.net/package/PHP_CodeSniffer
 */
class Mole_Sniffs_WhiteSpace_FunctionSpacingSniff implements PHP_CodeSniffer_Sniff
{
    ...
    public function process(PHP_CodeSniffer_File $phpcsFile, $stackPtr)
    {
        ...
        */
        ...
        if (isset($tokens[$stackPtr]['conditions']) === true && count($tokens[$stackPtr]['conditions']) === 1) {
            $conditions = $tokens[$stackPtr]['conditions'];
            $classPtr = key($conditions);
            if ($tokens[$classPtr]['code'] === T_CLASS
                && $tokens[$classPtr]['scope_closer']
                === $phpcsFile->findNext(T_CLOSE_CURLY_BRACKET, ($tokens[$stackPtr]['scope_closer'] + 1))
            ) {
                // PSR2.Classes.ClassDeclaration.CloseBraceAfterBody
//                if ($nextLineToken !== $nextContent) {
//                    $error = 'Expected 0 blank line between function closer and class closer; %s found';
//                    $data = array($foundLines);
//                    $fix = $phpcsFile->addFixableError($error, $tokens[$classPtr]['scope_closer'], 'NoBlankBetweenFunctionClass', $data);
//                    if ($fix === true) {
